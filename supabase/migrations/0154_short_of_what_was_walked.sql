-- 0154 — short of what was WALKED, not of what was drawn
--
-- Two corrections to the close-out line in the billing letter, both found by
-- reading 0153 back against the screen it is meant to agree with.
--
-- FIRST: the letter measured built feet against sum(fence_run.plan_ft). The
-- close-out screen measures against coalesce(walked_ft, plan_ft) — what it
-- calls sold — and that is the figure the project manager was made to justify
-- before the job could close at all. On any job where somebody walked a run and
-- found it longer or shorter than the aerial, the two would print different
-- numbers for "N ft short", in a document that gets filed and argued with. One
-- of them would be wrong and the letter is not the place to find that out.
--
-- SECOND: a fence_closeout row is written the moment somebody types who they
-- walked the job with, which can be days before any run carries a built figure.
-- The letter would have said "Built 0 ft". The line now waits until there is
-- something built to report.
--
-- Everything else in the function is 0153 unchanged.
