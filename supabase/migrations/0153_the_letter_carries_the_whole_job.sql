-- 0153 — the billing letter grows the job underneath it
--
-- Ryan, 14 Sep: the bottom of the billing email should carry the full project
-- log with full size pictures, the notes and changes, and full size copies of
-- the signed forms. And, separately: the sales rep at the top, so they get
-- credit for the sale.
--
-- WHY IT ALL HAS TO BE IN THE LETTER. Whoever keys this has no Hopper account.
-- A link into the app opens nothing for them, and the record filed against the
-- Navusoft account has to stand on its own six months later — the same argument
-- that put the keying sheet in the letter to begin with.
--
-- THE DEFINER STILL READS WHAT IT MAILS. Every one of the four new blocks is
-- built from rows this function looked up itself, from the job id it was given
-- and nothing else. Nothing is handed in. That rule is not decoration: a definer
-- that mails whatever text a caller hands it to whatever address a caller hands
-- it is a spam relay wearing our return address.
--
-- THE PICTURES ARE LINKED, NOT ATTACHED, AND THAT IS NOT A COMPROMISE I CHOSE.
-- beebee.mail_outbox has no attachment column, so a genuinely embedded image
-- would be a platform change touching every Oh hi app; and a data: URI is
-- stripped by Gmail and blocked outright by Outlook. A hosted image referenced
-- normally looks and prints identically. The src is composed here from the job's
-- own shot_token and the note's id, so the address in the letter is one this
-- function minted rather than one a caller supplied.
--
-- THE PAGES ARE READ, NOT REBUILT. hopper.fence_signature.page holds the
-- customer document as it read at the moment of signing. Re-deriving it here
-- would mean a second pricer written in plpgsql, drifting against the first on
-- a document somebody already put their name to. A signature with no frozen page
-- contributes no page at all, and the letter simply has fewer.
--
-- EVERY NEW KEY IS ABSENT WHEN IT IS EMPTY, so a job with no notes, no
-- signatures and no close-out queues precisely the letter it queues today.
...
begin
  ...
  select coalesce(sum(r.plan_ft), 0) into v_plan
  ...
  select jsonb_agg(s.c order by s.hap) into v_changes from (
    ...
    select coalesce(co.walked_on::timestamptz, co.closed_at) as hap,
     ...
     where co.account_id = v_acct and co.job_id = p_job
  ) s;
  ...
  values ('fence.handoff', 'hopper',
          ...
          jsonb_build_object(
            ...
            'url', v_base || '/fence/' || p_job::text || '/record',
